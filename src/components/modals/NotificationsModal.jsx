...
export default function NotificationsModal({ isOpen, onClose, sales, settings, clients, todayBirthdays, reminders, GOAL_SELLERS, GOAL_MANAGER, ELIGIBLE_FOR_GOAL, onGoToCalendar }) {
    ...
    const upcomingBirthdays = useMemo(() => {
        ...
        for (let i = 0; i <= 5; i++) {
            ...
            const dayLabel = i === 0 ? 'Hoje' : i === 1 ? 'Amanhã' : `Em ${i} dias`;
    ...
    return (
        <div className="fixed inset-0 z-[210] flex items-center justify-center p-4 bg-black/80 backdrop-blur-lg animate-in fade-in duration-500" onClick={onClose}>
            <div className="classic-frame rounded-[3rem] p-10 max-w-2xl w-full shadow-2xl animate-in zoom-in-95 relative overflow-hidden max-h-[85vh] overflow-y-auto custom-scrollbar" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-6">
                    {recentClients.length > 0 && (
                        <div className="p-6 bg-gradient-to-br from-amber-900/30 via-amber-800/20 to-yellow-900/20 border border-amber-400/30 rounded-[2rem] shadow-lg shadow-amber-500/10 animate-fade-in-up">
                            <div className="flex items-center gap-3 mb-4">
                                <span className="text-2xl">👤</span>
                                <span className="font-bold text-amber-300">Clientes Recém-Cadastrados</span>
                                <span className="ml-auto bg-amber-400/20 text-amber-300 text-xs font-bold px-3 py-1 rounded-full animate-pulse">{recentClients.length} novo{recentClients.length > 1 ? 's' : ''}</span>
                            </div>
                            <div className="space-y-3">
                                {recentClients.map((c, idx) => (
                                    <div key={c.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-amber-900/40 to-transparent rounded-xl border border-amber-500/20 hover:border-amber-400/40 transition-all hover:translate-x-1" style={{ animationDelay: `${idx * 100}ms` }}>
                    ...
                    <div className="p-6 bg-gradient-to-br from-slate-800/60 via-slate-800/40 to-slate-900/60 border border-slate-600/30 rounded-[2rem] shadow-lg animate-fade-in-up" style={{ animationDelay: '100ms' }}>
                        ...
                        <div className="space-y-4">
                            {sellerGoals.map((seller, idx) => (
                                <div key={seller.fullName} className="p-4 bg-gradient-to-r from-slate-900/80 to-transparent rounded-xl border border-slate-700/30 hover:border-slate-500/50 transition-all" style={{ animationDelay: `${(idx + 1) * 100}ms` }}>
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-2">
                                            <span className={`w-2 h-2 rounded-full animate-pulse ${seller.isManager ? 'bg-purple-400 shadow-purple-400/50' : 'bg-cyan-400 shadow-cyan-400/50'}`}></span>
                                            <span className="font-bold text-slate-200 text-sm">{seller.name}</span>
                                    ...
                                    {seller.needsAttention ? (
                                        ...
                                        <p className="text-xs text-slate-500">Progresso: {Math.round(seller.pct)}%</p>
                                    )}
                                </div>
                            ))}
                        </div>
                    ...
                    {upcomingBirthdays.length > 0 && (
                        <div className="p-6 bg-gradient-to-br from-pink-900/30 via-pink-800/20 to-purple-900/20 border border-pink-400/30 rounded-[2rem] shadow-lg shadow-pink-500/10 animate-fade-in-up" style={{ animationDelay: '200ms' }}>
                            <div className="flex items-center gap-3 mb-4">
                                <span className="text-2xl">🎂</span>
                                <span className="font-bold text-pink-300">Aniversários Próximos</span>
                                <span className="ml-auto bg-pink-400/20 text-pink-300 text-xs font-bold px-3 py-1 rounded-full animate-pulse">{upcomingBirthdays.length}</span>
                            </div>
                            <div className="space-y-3">
                                {upcomingBirthdays.map((c, idx) => (
                                    <div key={c.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-pink-900/40 to-transparent rounded-xl border border-pink-500/20 hover:border-pink-400/40 transition-all hover:translate-x-1" style={{ animationDelay: `${idx * 80}ms` }}>
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 bg-gradient-to-br from-pink-300 to-pink-500 rounded-full flex items-center justify-center text-pink-900 font-bold text-sm shadow-lg shadow-pink-500/30 animate-pulse">
                                                🎂
                                            </div>
                                            <div>
                                                <p className="font-bold text-slate-200 text-sm">{c.name}</p>
                                                <p className="text-xs text-pink-400/70 font-mono">{c.phone || 'Sem telefone'}</p>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2">
                                            <span className={`text-xs font-bold px-3 py-1 rounded-full ${c.daysUntil === 0 ? 'bg-green-500/30 text-green-300 animate-pulse' : 'bg-pink-900/40 text-pink-300'}`}>
                                                {c.dayLabel}
                                            </span>
                                            {c.phone && c.daysUntil <= 1 && (
                    ...
                    {todayReminders.length > 0 && (
                        <div className="p-6 bg-gradient-to-br from-orange-900/30 via-orange-800/20 to-amber-900/20 border border-orange-400/30 rounded-[2rem] shadow-lg shadow-orange-500/10 animate-fade-in-up" style={{ animationDelay: '300ms' }}>
                            <div className="flex items-center gap-3 mb-4">
                                <span className="text-2xl">⏰</span>
                            ...
                            <div className="space-y-3">
                                {todayReminders.map((r, idx) => (
                                    <div key={r.id} className="flex items-center justify-between p-4 bg-gradient-to-r from-orange-900/40 to-transparent rounded-xl border border-orange-500/20 hover:border-orange-400/40 transition-all" style={{ animationDelay: `${idx * 80}ms` }}>
                                        <div>
                ...
                <button onClick={onGoToCalendar} className="w-full mt-8 py-4 bg-gradient-to-r from-amber-300 via-yellow-200 to-amber-300 text-amber-900 rounded-[2rem] font-black hover:shadow-lg hover:shadow-amber-400/40 transition-all active:scale-[0.98] hover:scale-[1.01] animate-fade-in-up" style={{ animationDelay: '400ms' }}>
                    Ver Calendário
                </button>
            </div>
        </div>
