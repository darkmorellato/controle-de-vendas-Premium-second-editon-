...
import { analyzeReferrals } from '../../utils/referralClustering.js';
...
const ReferralsView = ({ sales, formatCurrency, formatDateBR, monthFilter, setMonthFilter, availableMonths }) => {
    ...
    const salesWithSource = filteredSales.filter(s => s.clientSource && s.clientSource.trim());
    const analysis = analyzeReferrals(salesWithSource, filteredSales);
    const { sources, totalReferrals, totalRevenue, topSource, maxCount, maxRev, referralRate, mergedGroups, srcToCanonical } = analysis;
            ...
            <div className="classic-frame rounded-[3rem] shadow-vision border border-white/60 p-10">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
                    ...
                    <MonthFilterDropdown
                        ...
                        dropdownClassName="referrals-month-filter-dropdown"
                        showButtonText
                    />
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
                    {kpis.map((k) => (
                        <div key={k.label} className={`p-5 rounded-[2rem] border ${k.bg} hover:-translate-y-2 hover:shadow-xl ${k.glow} transition-all duration-300 group`}>
                ...
                {sources.length > 0 ? (
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-amber-500/10">
                                        {[...salesWithSource].sort((a, b) => (b.date || '').localeCompare(a.date || '')).map((s) => {
                                            ...
                                            return (
                                                <tr key={s.id} className="hover:bg-amber-500/10 transition-all duration-300 group">
                    ...
                    <div className="text-center py-24 text-slate-500 flex flex-col items-center">
                        ...
                        <p className="font-bold text-2xl text-slate-400 mb-2">
                            {monthFilter !== 'todos' ? 'Nenhuma indicação neste mês' : 'Nenhuma indicação registrada'}
                        </p>
                        <p className="text-sm">
                            {monthFilter !== 'todos' 
                                ? 'Selecione outro mês ou "Todos os Meses" para ver todas.' 
                                : 'Preencha o campo "Conheceu a loja?" ao registrar uma venda.'}
                        </p>
                    </div>
                )}
            </div>
...
export default memo(ReferralsView);